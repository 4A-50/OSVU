...
public class Hand_OLD : MonoBehaviour
{
    ...
    Interactable_OLD currentInteractable = null;
    //All The Nearby Interactables
    List<Interactable_OLD> contactInteractables = new List<Interactable_OLD>();
    //Nearest Interactable
    Interactable_OLD nearestInteractable = null;
    ...
    private void Update()
    {
        if (contactInteractables.Count > 0)
        {
            ...
            {
                if (grabAction.GetStateDown(pose.inputSource) && nearestInteractable.holdType == Interactable_OLD.PickUpType.Toggle)
                {
                ...
                else if (nearestInteractable.holdType == Interactable_OLD.PickUpType.Hold)
                {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        contactInteractables.Add(other.gameObject.GetComponent<Interactable_OLD>());
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        contactInteractables.Remove(other.gameObject.GetComponent<Interactable_OLD>());
    }
    ...
    private Interactable_OLD GetNearestInteractable()
    {
        Interactable_OLD nearest = null;
        float minDistance = float.MaxValue;
        float distance = 0f;

        foreach (Interactable_OLD interactable in contactInteractables)
        {
